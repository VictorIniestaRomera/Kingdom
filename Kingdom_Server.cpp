...
std::vector<connection> playerList;
...
bool listen_socket(SOCKET getSocket) {
    ...
    cout << "Socket escuchando. Esperando data entrante..." << endl;
...
int main() {
    ...
    while (loop) {
        ...
        if (data != SOCKET_ERROR) {
            ...
            Message mssg;
            mssg.type = SET_TEAM;
            mssg.setID = playerList.size();

            char* b = (char*)&mssg;
            send_data(data, b, sizeof(MSG));
        }
    }
